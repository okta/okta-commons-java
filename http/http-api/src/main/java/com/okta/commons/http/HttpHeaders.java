...
import com.okta.commons.lang.LinkedCaseInsensitiveMap;
...
public final class HttpHeaders implements Map<String, List<String>> {
    ...
    private static Entry<String, String> parseLinkHeader(String rawHeader) {
        Pattern pattern = Pattern.compile("\\<(.*)\\>;.*rel=\"(.*)\"");
        Matcher matcher = pattern.matcher(rawHeader);
        if (matcher.matches()) {
            return new AbstractMap.SimpleEntry<>(matcher.group(2), matcher.group(1));
        }
        return null;
    }
